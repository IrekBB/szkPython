...
class IndeksNazwisk:        # Klasa pomocnicza, realizuje sortowanie wg nazwisk
    ...
    def szukajRefN(self,nazwisko):
        tmp = self
        znaleziono = False
        while tmp != None:
            if tmp.ref.nazwisko == nazwisko:
                znaleziono = True
                break
            tmp = tmp.nastepny
        return znaleziono, tmp
# --- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
# ---------------------------------------------------------------------------------
    ...
    def usunZIndeksuNazwisko(self, nazwisko):
        znaleziono, ref = self.glowaN.szukajRefN(nazwisko)
        if znaleziono:
            przed = None  
            po = self.glowaN
            while (po != None):
    ...
    def usunZIndeksuZarobek(self, nazwisko):
        znaleziono, ref = self.glowaZ.szukajRefZ(nazwisko)
        if znaleziono:
            przed = None  
            po = self.glowaZ
            while (po != None):
