...
class Kartoteka:    # Ta klasa realizuje podstawową listę jednokierunkową zawierającą rekordy klasy 'Osoba'
    ...
    def wstawNaKoniec(self, pNazwisko, pZarobek):   # Wstawiamy dane w kolejności przychodzenia
        ...
        return osoba                      # Wazna instrukcja, zwraca referencje do nowo yworzonego obiektu klasy Osoba
                                          # Będziemy z niej korzystać podczas przebudowy indeksów, czyli list-wskaźników realizujących sortowanie danych
    # -------------------------------------------------------------------------
    def wypisz(self):
        tmp=self.glowa
        if tmp == None:
    ...
    def usunWybrany(self, nazwisko):  # Odszukaj i usuń osobę o nazwisku 'nazw' na liście
        ...
        if znaleziono == False:
            print("Nie znaleziono osoby o nazwisku", nazwisko)
            return
