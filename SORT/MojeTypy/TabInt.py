...
class TabInt:	# Klasa zarządzająca umowną tablicą używana w rozdziale "Algorytmy sortowania"
	...
	def bubble(self):    # Algorytm sortowania bąbelkowego
		for i in range (1, self.rozmiar):
			for j in range (self.rozmiar-1, i-1, -1):  # Od rozmiar - 1 do i (pętla wsteczna)
				if self.t[j] < self.t[j-1]:
					temp = self.t[j-1]
					self.t[j-1] = self.t[j]
					self.t[j] = temp
	...
	def QuickSort(self, left, right):       # Sortowanie szybkie (QickSorting)
		if (left < right):
			m = left
...
if __name__ == "__main__":
    print("To jest moduł biblioteczny, aby przetestować wywołaj plik:", "MainTabInt.py")
