...
def main(args):
    ...
    print (f" Czas --> Godzina: {teraz.hour}, minuta: {teraz.minute}, selunda: {teraz.second}")
    print ("Modyfikacja składowych (miesiąc ustawiamy na 10, czyli październik)")
    teraz.replace(month=10)
    print ("Obiekt po modyfikacji")
    print (f" Data --> Dzień: {teraz.day}, miesiąc: {teraz.month}, rok: {teraz.year} ")
    print("Operacje arytmetyczne  na obiektach 'datetime':")
    d1 = datetime.datetime(2021,5,1,0,0,0)   # Określony moment w czasie (1 maja 2021)
    print (" Data 1:                                      ", str(d1))
    d3 = datetime.datetime(2021, 5,3,0,0,0)  # Okreslony moment w czasie (3 maja 2021 r.)
    print (" Data 2:                                      ", str(d3))
    ...
    print (teraz.strftime("%d.%m.%Y")," Nr tygodnia:", teraz.strftime("%U"), " (teraz)")
    delta = datetime.timedelta(days=25)
    print ((teraz+delta).strftime("%d.%m.%Y")," Nr tygodnia:", (teraz+delta).strftime("%U"), " (teraz + 25 dni)")
