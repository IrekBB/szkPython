# 6102025 0731
# USS - uniwersalna struktura słownikowa
# Umawiamy się, że zapętlenie wskaźnika t[n-1] do swojej własnej tablicy
# oznacza znacznik końca słowa

ROZM = 29    # Rozmiar alfabetu

def do_indeksu(c):  # Konwersja znaku 'c na wartość 0..25 dla liter z zakresu A..Z, 26 dla spacji
    if (c >= 'A' and c <='Z') or (c >= 'a' and c <= 'z'):  # i 27 dla znaku '-' (minus)
        return ord(c.upper()) - ord('A')     # Zamiana na dużą literę realizowana przez metode upper(), kod 'A' to 65 
    elif c==' ':
        return 26
    elif c=='-':
        return 27
...
def z_indeksu(n):   # Konwersja indeksu z zakresu 0..27 na odpowiedni znak alfabetu
    if n >= 0 and n <= (ord('Z')- ord('A')):
        ...
        return '@'   # Umowny kod błędu
    

class USS:
    def __init__(self):
        self.t=[None for _ in range(ROZM)] # W tablicy będziemy zapisywali referencje do obiektów USS
    
    def zapisz(self, slowo):
        # Zmienne pomocnicze pozwolą nam nawigować po drzewie wskazywanym przez referencję
        # do własnego obiektu (self)
        p = self
        ...
        for i in range(ROZM):
            if self.t[i] != None:
                if (self.t[i]).t[ROZM-1] == self.t[i]:
                    print (z_indeksu(i))   # Gdy koniec słowa, pisz znak końca linii
                else:
...
def main(args):
    p = USS()   # Tworzymy nowy słownik i wpisujemy kilka haseł
